...
# Load original track
root = Path("/Users/egg/Library/CloudStorage/Dropbox/music/")
bloom_path = root / "radiohead" / "Bloom.m4a"
song = AudioSegment.from_file(bloom_path)
...
samp_1  = song[(0*H)+(0*M)+(1*S)+(800*I):(0*H)+(0*M)+(3*S)+(350*I)]
samp_2   = song[(0*H)+(0*M)+(1*S)+(1*I):(0*H)+(0*M)+(1*S)+(1*I)]
samp_3  = song[(0*H)+(0*M)+(1*S)+(1*I):(0*H)+(0*M)+(1*S)+(1*I)]

# play(samp_1)
# play(samp_2)
# play(samp_3)

samp = samp_1 + samp_1 + samp_1
