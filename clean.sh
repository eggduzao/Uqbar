...
find "$FIND_ROOT" $SKIP_GIT -type d -name "__pycache__" -prune -exec rm -rf {} + 2>/dev/null || true
...
find "$FIND_ROOT" $SKIP_GIT -type d -name "*.egg-info" -prune -exec rm -rf {} + 2>/dev/null || true
...
# Jupyter checkpoints
find "$FIND_ROOT" $SKIP_GIT -type d -name ".ipynb_checkpoints" -prune -exec rm -rf {} + 2>/dev/null || true

# Sphinx doctrees (if outside _build)
find ./docs $SKIP_GIT -type d -name ".doctrees" -prune -exec rm -rf {} + 2>/dev/null || true
...
find "$FIND_ROOT" $SKIP_GIT -type d -name ".Trashes" -prune -exec rm -rf {} + 2>/dev/null || true
